...
# Arguments
- `output::String`: Raw solver output string.
...
"""
    _build_quantifier_expr(quantifier::Symbol, vars, body) -> String

Internal helper that directly builds the SMT-LIB2 string for a quantified
expression, since quantifier binding syntax doesn't map cleanly to Julia `Expr`.

Returns a raw SMT-LIB2 string that can be stored as-is in an assertion.

# Arguments
- `quantifier::Symbol`: Either `:forall` or `:exists`.
- `vars`: Vector of `name => Type` pairs.
- `body`: Julia expression for the body.

# Returns
An SMT-LIB2 string like `"(forall ((x Int)) (> x 0))"`.
"""
function _build_quantifier_expr(quantifier::Symbol, vars, body)
    smt_quant = quantifier == :forall ? "forall" : "exists"
    bindings = join(["($(v.first) $(smt_type(v.second)))" for v in vars], " ")
    smt_body = to_smtlib(body)
    "($smt_quant ($bindings) $smt_body)"
end

# Override assert! to handle raw SMT-LIB strings from quantifier builders
"""
    assert!(ctx::SMTContext, expr::AbstractString; name=nothing)

Add a raw SMT-LIB2 expression string as an assertion to the context.

This overload is used when the expression is already in SMT-LIB2 format
(e.g. from `forall` or `exists` builders).

# Arguments
- `ctx::SMTContext`: The solving context.
- `expr::AbstractString`: Raw SMT-LIB2 expression string.
- `name::Union{Symbol,Nothing}`: Optional label for unsat cores.
"""
function assert!(ctx::SMTContext, expr::AbstractString; name::Union{Symbol,Nothing}=nothing)
    if name !== nothing
        named_expr = "(! $expr :named $name)"
        assertion = "(assert $named_expr)"
        ctx.assertion_names[name] = assertion
    else
...
# ============================================================================
# Optimization (Z3-specific)
# ============================================================================

"""
    minimize!(ctx::SMTContext, expr)

Add a minimization objective to the context (Z3-specific).

When `optimize(ctx)` is called, the solver will attempt to find a model
that minimizes the value of `expr` while satisfying all assertions.

Note: Optimization is only supported by Z3. Other solvers will ignore or
reject optimization directives.

# Arguments
- `ctx::SMTContext`: The solving context.
- `expr`: Julia expression to minimize.
...
minimize!(ctx, :x)
result = optimize(ctx)
# result.model[:x] should be 1 (smallest x > 0)
```
"""
function minimize!(ctx::SMTContext, expr)
    smt_expr = expr isa AbstractString ? expr : to_smtlib(expr)
...
Add a maximization objective to the context (Z3-specific).

When `optimize(ctx)` is called, the solver will attempt to find a model
that maximizes the value of `expr` while satisfying all assertions.

Note: Optimization is only supported by Z3.

# Arguments
- `ctx::SMTContext`: The solving context.
- `expr`: Julia expression to maximize.
...
maximize!(ctx, :x)
result = optimize(ctx)
# result.model[:x] should be 99 (largest x < 100)
```
"""
...
"""
    optimize(ctx::SMTContext; get_model=true) -> SMTResult

Run the solver in optimization mode (Z3-specific).

Similar to `check_sat`, but includes any minimization/maximization directives
that were added via `minimize!` and `maximize!`. The solver will find an
optimal model (if satisfiable) according to the optimization objectives.

Note: This function requires Z3 as the solver. CVC5 and other solvers do
not support the `(minimize ...)` / `(maximize ...)` directives.

# Arguments
- `ctx::SMTContext`: The solving context with optimization directives.
- `get_model::Bool`: Request model on sat (default `true`).

# Returns
An `SMTResult` with the optimal model if satisfiable.

# Example
```julia
ctx = SMTContext(logic=:QF_LIA)
declare(ctx, :cost, Int)
assert!(ctx, :(cost >= 0))
...
```
"""
function optimize(ctx::SMTContext; get_model::Bool=true)
...
"""
    bv(val::Integer, width::Integer) -> String

Construct an SMT-LIB2 bitvector literal string.

Produces the indexed bitvector literal `(_ bvN width)` where `N` is the
decimal value and `width` is the bit-width.

# Arguments
- `val::Integer`: The bitvector value (non-negative).
- `width::Integer`: The bit-width.

# Returns
An SMT-LIB2 bitvector literal string.
...
```
"""
function bv(val::Integer, width::Integer)
    val < 0 && error("Bitvector value must be non-negative, got $val")
    width < 1 && error("Bitvector width must be >= 1, got $width")
    "(_ bv$val $width)"
end

"""
    fp_sort(ebits::Integer, sbits::Integer) -> String

Construct an SMT-LIB2 floating-point sort string.

Produces `(_ FloatingPoint ebits sbits)` which specifies an IEEE 754
floating-point type with `ebits` exponent bits and `sbits` significand bits
(including the hidden bit).

Common configurations:
- `fp_sort(5, 11)` = IEEE 754 half precision (Float16)
- `fp_sort(8, 24)` = IEEE 754 single precision (Float32)
- `fp_sort(11, 53)` = IEEE 754 double precision (Float64)

# Arguments
- `ebits::Integer`: Number of exponent bits.
- `sbits::Integer`: Number of significand bits.

# Returns
An SMT-LIB2 sort string.

# Example
```julia
fp_sort(8, 24)    # => "(_ FloatingPoint 8 24)"   (Float32)
fp_sort(11, 53)   # => "(_ FloatingPoint 11 53)"  (Float64)
```
"""
function fp_sort(ebits::Integer, sbits::Integer)
    ebits < 1 && error("Exponent bits must be >= 1, got $ebits")
    sbits < 1 && error("Significand bits must be >= 1, got $sbits")
    "(_ FloatingPoint $ebits $sbits)"
end

"""
    array_sort(idx_type::Type, elem_type::Type) -> String

Construct an SMT-LIB2 array sort string from Julia types.

Produces `(Array idx_sort elem_sort)`.

# Arguments
- `idx_type::Type`: Julia type for the index (e.g. `Int`, `BitVec{32}`).
- `elem_type::Type`: Julia type for the element (e.g. `Bool`, `Int`).
...
# Example
```julia
array_sort(Int, Bool)           # => "(Array Int Bool)"
array_sort(BitVec{32}, Int)     # => "(Array (_ BitVec 32) Int)"
```
"""
function array_sort(idx_type::Type, elem_type::Type)
    "(Array $(smt_type(idx_type)) $(smt_type(elem_type)))"
end

"""
    re_sort() -> String

Return the SMT-LIB2 sort for regular expressions over strings.

In SMT-LIB2, the regex sort is `RegLan` (regular language).
...
Parse solver statistics from the raw output of an `SMTResult`.

Looks for `:keyword value` pairs in the solver output, which is the format
used by Z3's `(get-info :all-statistics)` output and also appears in some
solver verbose modes.

If the result already has parsed statistics, returns those. Otherwise,
attempts to parse from raw output.

# Arguments
- `result::SMTResult`: The solver result.

# Returns
A `Dict{String,Any}` of statistic name => value pairs. Values are `Float64`
for numeric statistics or `String` otherwise.

# Example
```julia
result = check_sat(ctx)
stats = get_statistics(result)
# stats might contain: "memory" => 12.5, "time" => 0.01, etc.
```
"""
function get_statistics(result::SMTResult)
    ...
    # Parse :keyword value pairs from output
    # Common format: :stat-name  value
    for m in eachmatch(r":([a-zA-Z0-9_\-\.]+)\s+([^\s:)]+)", output)
        key = m.captures[1]
        val_str = m.captures[2]

        # Try to parse as number
        value = try
            # Try Float64 first (covers integers too)
            if occursin('.', val_str)
...
"""
    evaluate(model::Dict{Symbol,Any}, expr) -> Any

Substitute model values into a Julia expression and evaluate it.

Replaces all symbols in `expr` that appear as keys in `model` with their
corresponding values, then evaluates the resulting expression.

# Arguments
- `model::Dict{Symbol,Any}`: Variable assignments from an `SMTResult.model`.
- `expr`: Julia expression to evaluate.

# Returns
The evaluated result.

# Throws
- `ErrorException` if evaluation fails (e.g. undefined variables not in model).

# Example
```julia
model = Dict(:x => 3, :y => 7)
evaluate(model, :(x + y))     # => 10
evaluate(model, :(x * y > 0)) # => true
evaluate(model, :(x - 1))     # => 2
```
"""
function evaluate(model::Dict{Symbol,Any}, expr)
    substituted = _substitute(model, expr)
    try
        # Evaluate the substituted expression
        if substituted isa Expr
            return Base.eval(substituted)
        else
            return substituted
        end
    catch e
        error("Failed to evaluate expression: $substituted ($e)")
    end
end

"""
    _substitute(model::Dict{Symbol,Any}, expr) -> Any

Recursively substitute model values into an expression. Internal helper
for `evaluate`.
...
# Returns
The expression with all model variables replaced by their values.
"""
function _substitute(model::Dict{Symbol,Any}, expr)
    if expr isa Symbol
        return get(model, expr, expr)
    elseif expr isa Expr
        new_args = [_substitute(model, a) for a in expr.args]
...
Run the solver with unsat core extraction and return the named assertion labels
that form the unsatisfiable core.

This function sets the `produce-unsat-cores` option, runs `check_sat` with
core extraction enabled, and returns the parsed core labels. Only meaningful
when assertions have been named via `assert!(ctx, expr; name=:label)`.

# Arguments
- `ctx::SMTContext`: The solving context with named assertions.

# Returns
A `Vector{Symbol}` of assertion labels in the unsat core. Empty if the
problem is satisfiable or no core was produced.
...
core = get_unsat_core(ctx)
# core should contain :pos and :neg
```
"""
function get_unsat_core(ctx::SMTContext)
    result = check_sat(ctx; get_model=false, get_unsat_core=true)
    result.unsat_core
end
