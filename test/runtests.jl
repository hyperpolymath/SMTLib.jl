  ...
  (define-fun q () Bool false)
    ...
    @testset "Theory Helpers" begin
        ...
        @testset "Floating-point sorts" begin
            @test fp_sort(8, 24) == "(_ FloatingPoint 8 24)"     # Float32
            @test fp_sort(11, 53) == "(_ FloatingPoint 11 53)"   # Float64
            @test fp_sort(5, 11) == "(_ FloatingPoint 5 11)"     # Float16
        ...
        @testset "Array sorts" begin
            @test array_sort(Int, Bool) == "(Array Int Bool)"
            @test array_sort(Int, Int) == "(Array Int Int)"
            @test array_sort(SMTLib.BitVec{32}, Int) == "(Array (_ BitVec 32) Int)"
            @test array_sort(Bool, Float64) == "(Array Bool Real)"
        end
    ...
    @testset "Quantifiers" begin
        @testset "forall construction" begin
            result = forall([:x => Int, :y => Int], :(x + y == y + x))
                ...
                @testset "Solver $(solver.kind)" begin
                    ...
                    if solver.kind == :z3
                        @testset "Unsat core extraction (Z3)" begin
                            ...
                            # The result might vary by solver version
